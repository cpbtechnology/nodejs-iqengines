...
function makeSignature(fields, files, filename, secret)
{
    var joined      = [],
        rawstring   = '',
...
var Api = (function(){
    ...
    cls.prototype._signedRequest = function (args, callback) {
        var d = { url: null, method: "POST", fields: null, files: null },
            ...
            sig,
            filename  = U.clone(args.filename);

        // add file fields
        fields.push(["api_key", this.api_key]);
        fields.push(["time_stamp", makeTimestamp()]);

        sig = makeSignature(fields, files, filename, this.api_secret);
        fields.push(["api_sig", sig]);

        // send request
        req = srequest.SimpleRequest(args.url, args.method, fields, files, filename, callback);
        return sig;
    };
