...
function makeMultipartFormData (fields, files, filename) {
    ...
    var BOUNDARY = '----------ThIs_Is_tHe_bouNdaRY_$',
        ...
        for (var i     = 0; i < files.length; i++) {
            ...
            L.push(toUTF8Buff('--' + BOUNDARY));
            L.push(toUTF8Buff('Content-Disposition: form-data; name="' + key + '"; filename="' + filename + '"'));
            L.push(toUTF8Buff('Content-Type: application/octet-stream'));
            L.push(toUTF8Buff(''));
            L.push(val); // Already a buffer
        };
...
function SimpleRequest(url, method, fields, files, filename, callback) {
    ...
    // Send multipart/form-data data in single chunk
        ...
        formdata = makeMultipartFormData(fields, files, filename);
